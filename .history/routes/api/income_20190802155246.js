...
incomeRouter.post('/income',  async (req, res) => {
    ...
    try {
     ...
     Income.create({
           ...
           User.findOne({email: 'twentekghana@gmail.com'}, function(err, foundUser){
                 if(err){
                      return console.log('No user found to create this income')
                 }else {
...
//       User.findOne({email: "e.tweneboah1@gmail.com"}).populate("incomes").exec(function(err, user){
//     if(err){
