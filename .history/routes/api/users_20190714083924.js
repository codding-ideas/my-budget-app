...
check('password', 'Password is required with minimum length of 4').isLength({min: 4})
  ...
  if(!errors.isEmpty()){
    ...
    try {
      ...
      if(user){
        res.status(400).json({errors: [{msg: 'User already exisit'}]}) //Matching errors from the error array, thus either is from name, email or password
      }
      //1. Destructure
  const { name, email, password } = req.body
  ...
  user.password = await bcrypt.hash(pas)
