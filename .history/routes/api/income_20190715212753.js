...
incomeRouter.get('/me', auth, async (req, res) => {
      try {
            const income =  await Income.findOne().populate('user', ['name', 'email']) //Searching if there is authenticated user in the income model. If there is a user then we will add the user's details to this request by using populate
...
incomeRouter.post('/', [
      ...
      try {
          ...
          let  income = new Income({incomeAmount, description})
