...
usersRouter.get('/', async(req, res) => {
  try {
      const allUsers = await User.find().populate('income').exec(function(err, user) {
          return res.json()
      });
...
//Get All Users and populate it's income
usersRouter.get('/users',   async (req, res) => {
   try {
         await User.find().populate('income').exec(function(err, user) {
            res.json({
               user
            })
         })
