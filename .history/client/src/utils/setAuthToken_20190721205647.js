...
// // When a user is created, a token is generated for that user and since REST is stateless we have to take that token an put it into a global header so that anytime we hit an endpoint that requires token to return the user back and this will help us to determined if a user is logged in or not
