...
const PrivateRoute = ({component: Component, auth, ...rest}) => (
   ...
   //The render function says if the props passed in (redux state) is not authenticated or loading then redirect the user to login page otherwise render the component passed in. The isAthenticated and loading are from redux

   //Next we have to pass in any route/component we need to protect to this function so let's move over to app.js

  <Route {...rest} render = {(props => {
     return props.loginUser.isAuthenticated === null ? <Redirect to ='/login' /> : <Component {...props}/>
  })} />
)
